...
const Index = () => {
    ...
    const [storageAdapter, setStorageAdapter] = useState<StorageAdapter>(() => {
        ...
        if (storageType === "indexedDB") return new IndexedDBAdapter();
        if (storageType === "localDirectory") return new FileSystemAdapter();
    ...
    useEffect(() => {
        const handleStorageChange = async () => {
            const storageType = (localStorage.getItem(STORAGE_TYPE_KEY) || "localStorage") as StorageType;
            let newAdapter: StorageAdapter;
            if (storageType === "indexedDB") newAdapter = new IndexedDBAdapter();
            else if (storageType === "localDirectory") newAdapter = new FileSystemAdapter();
            else newAdapter = new LocalStorageAdapter();
    ...
    const openFiles: OpenFile[] = appState.openFiles
        .map(({ fileId, projectId }) => {
    ...
    const handleFileSelect = (projectId: string, fileId: string) => {
        setAppState((prev) => {
            ...
            const newOpenFiles = isOpen ? prev.openFiles : [...prev.openFiles, { fileId, projectId }];
    ...
    const handleFileDuplicate = (projectId: string, fileId: string) => {
        ...
        const updatedProjects = appState.projects.map((p) => (p.id === projectId ? { ...p, files: [...p.files, newFile] } : p));
    ...
    const handleContentChange = (content: any) => {
        ...
        const updatedProjects = appState.projects.map((project) =>
            project.id === appState.currentProjectId
                ? {
                      ...project,
                      files: project.files.map((file) => (file.id === appState.currentFileId ? { ...file, content, updatedAt: Date.now() } : file)),
                  }
                : project
        );
